  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="状态" prop="status">
            <el-select v-model="queryParams.status" placeholder="护理项目状态" clearable style="width: 200px">
               <el-option
    ...
    <el-table v-loading="loading" :data="projectList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="护理项目名称" align="center" prop="name" />
      ...
      <el-table-column label="操作" fixed="right" width="200" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-button v-if="scope.row.status==0"link type="primary" icon="Unlock" @click="handleEnable(scope.row)" v-hasPermi="['nursing:project:remove']">启用</el-button>
          <el-button v-else link type="primary" icon="Lock" @click="handleEnable(scope.row)" v-hasPermi="['nursing:project:remove']">停用</el-button>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="projectRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="排序号" prop="orderNo">
          <el-input-number min="1" max="20" v-model="form.orderNo" placeholder="请输入排序号" />
        </el-form-item>
        <el-form-item label="单位" prop="unit">
          <el-input v-model="form.unit" placeholder="请输入单位" />
        </el-form-item>
        <el-form-item label="价格" prop="price">
          <el-input-number min="1" step="1" v-model="form.price" placeholder="请输入价格" />
        </el-form-item>
        <el-form-item label="状态" prop="status">
...
function handleEnable(row){
  ...
  // proxy: 代理对象 vue
  proxy.$modal.confirm(`确认要【${text}】${row.name} 项目吗?`).then(function () {
    return updateProject({id:row.id, status: status=='0'?1:0});
  }).then(() => {
    getList();
